...
center_header_line() {
    ...
    text_width=$(printf '%s' "$prefix$text" | wc -L)
    padding=$((inner_width - text_width))
    left_padding=$((padding / 2))
    right_padding=$((padding - left_padding))

    printf '#%*s%s%b%s%b%*s#\n' \
        "$left_padding" '' "$prefix" "$color" "$text" "$PLAIN" "$right_padding" ''
}
...
APP_IMPORT_GUIDE="Open 'HTTP Tweak' app -> Add Server -> Tunnel Type set 'Hysteria' -> Paste Hysteria config"
...
write_client_config(){
    ...
    server=$(grep -m1 '"server":' /root/hy/hy-client.json | cut -d '"' -f4)
    obfs=$(grep -m1 '"obfs":' /root/hy/hy-client.json | cut -d '"' -f4)
    auth=$(grep -m1 '"auth_str":' /root/hy/hy-client.json | cut -d '"' -f4)

    server_host="${server%:*}"
    server_ports="${server##*:}"
    if [[ $server_ports == *,* ]]; then
        client_port="${server_ports#*,}"
    else
        client_port=$server_ports
    fi

    printf '%s:%s@%s:%s\n' "$server_host" "$client_port" "$obfs" "$auth" > /root/hy/config.txt
}
...
installHysteria(){
    ...
    wget -N https://raw.githubusercontent.com/Wyr-Hub/hysteria-install/main/hy1/install_server.sh
...
EOF
    ...
    yellow "Hysteria config is as follows and saved to /root/hy/config.txt"
    red "$(cat /root/hy/config.txt)"
}
...
change_port(){
    ...
    old_server=$(grep -m1 '"server":' /root/hy/hy-client.json | cut -d '"' -f4)
    server_host="${old_server%:*}"
    inst_port
    ...
    sed -i "s|\"server\": \".*\"|\"server\": \"$server_host:$last_port\"|" /root/hy/hy-client.json
    sed -i "s/^[[:space:]]*port: .*/    port: $port/" /root/hy/clash-meta.yaml
    write_client_config
...
menu() {
    clear
    echo "###############################################################################"
    center_header_line "$YELLOW" "⚡ " "Hysteria (ver 1) || one-click installation script ||"
    center_header_line "$RED" "" "https://github.com/Wyr-Hub/hysteria-install"
    echo "###############################################################################"
    echo ""
