...
APP_IMPORT_GUIDE="Open 'HTTP Tweak' app -> Add Server -> Tunnel Type set 'V2Ray' -> Protocol set 'Hysteria2' -> Paste Hysteria config"
...
installHysteria(){
    ...
    wget -N https://raw.githubusercontent.com/Wyr-Hub/hysteria-install/main/hy2/install_server.sh
...
EOF
    ...
    url="hy2://$auth_pwd@$ip:$last_port/?insecure=1&sni=$hy_host&$obfs_uri#HttpTweak-hysteria2"
    echo $url > /root/hy/url.txt
    nohopurl="hy2://$auth_pwd@$ip:$port/?insecure=1&sni=$hy_host&$obfs_uri#HttpTweak-hysteria2"
...
menu() {
    clear
    echo "###############################################################################"
    center_header_line "$YELLOW" "⚡ ⚡ " "Hysteria || one-click installation script ||"
